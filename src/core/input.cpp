...
{
    // Scroll up decreases slot, scroll down increases slot
    state.selectedSlot -= (int)yoffset;
    
    // Wrap around
    if (state.selectedSlot < 0) state.selectedSlot = 8;
    if (state.selectedSlot > 8) state.selectedSlot = 0;
}
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) state.selectedSlot = 0;
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) state.selectedSlot = 1;
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) state.selectedSlot = 2;
    if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS) state.selectedSlot = 3;
    if (glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS) state.selectedSlot = 4;
    if (glfwGetKey(window, GLFW_KEY_6) == GLFW_PRESS) state.selectedSlot = 5;
    if (glfwGetKey(window, GLFW_KEY_7) == GLFW_PRESS) state.selectedSlot = 6;
    if (glfwGetKey(window, GLFW_KEY_8) == GLFW_PRESS) state.selectedSlot = 7;
    if (glfwGetKey(window, GLFW_KEY_9) == GLFW_PRESS) state.selectedSlot = 8;
}