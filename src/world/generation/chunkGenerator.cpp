    ...
    for (int x = 0; x < CHUNK_SIZE; x++) {
        for (int z = 0; z < CHUNK_SIZE; z++) {
            for (int y = 0; y < CHUNK_HEIGHT; y++) {
                int worldY = y + Y_MIN;
                if (worldY == Y_MIN) chunk.setBlock(x, y, z, Blocks::Bedrock);
                else if (worldY < 60) chunk.setBlock(x, y, z, Blocks::Stone);
                else if (worldY < 63) chunk.setBlock(x, y, z, Blocks::Dirt);
                else if (worldY == 63) chunk.setBlock(x, y, z, Blocks::Grass);
            }
        }
    }
}

void ChunkGenerator::makeSeeded(Chunk& chunk, siv::PerlinNoise perlin, siv::PerlinNoise continental) {
    for (int x = 0; x < CHUNK_SIZE; x++) {
        for (int z = 0; z < CHUNK_SIZE; z++) {
            ...
            if (height > Y_MAX - 1) height = Y_MAX - 1;
